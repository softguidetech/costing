...
class MrpBom(models.Model):
    ...
    def _update_bom_costs(self):
        # Unlink existing material costs
        self.bom_material_cost_ids.unlink()

        # Create material costs
        material_cost_vals = []
        for material in self.bom_line_ids:
            product_id = self.env['product.template'].search([('id', '=', material.product_id.id)])
            vals = {
            ...
            material_cost_vals.append((0, 0, vals))

        # Update or create material costs
        self.bom_material_cost_ids = material_cost_vals

        config = self.env['res.config.settings'].search([], order="id desc", limit=1)
        operation_ids = self.env['mrp.routing.workcenter'].search([])

        # Check process costing method
        if operation_ids and config.process_costing == 'workcenter':
            # Unlink existing labor and overhead costs
            self.bom_labour_cost_ids.unlink()
            self.bom_overhead_cost_ids.unlink()

            labour_cost_vals = []
            overhead_cost_vals = []

            # Create labor and overhead costs
            for operation in operation_ids:
                ...
                labour_cost_vals.append((0, 0, labour_vals))
                ...
                overhead_cost_vals.append((0, 0, overhead_vals))

            # Update or create labor and overhead costs
            self.bom_labour_cost_ids = labour_cost_vals
            self.bom_overhead_cost_ids = overhead_cost_vals

    @api.model
    def create(self, vals):
        res = super(MrpBom, self).create(vals)
        res._update_bom_costs()
        return res

    def write(self, vals):
        res = super(MrpBom, self).write(vals)
        if 'bom_line_ids' in vals or 'routing_id' in vals:
            self._update_bom_costs()
        return res
    ...
    #         product_ids = self.env['product.template'].search([('id', '=', material.product_id.id)])
    #         if product_ids:
    ...
    #                 }
    #                 material_obj = self.env['mrp.bom.material.cost'].create(vls)
    #                 print(material_obj,"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
    #
    #     config = self.env['res.config.settings'].search([], order="id desc", limit=1)
    #     operation_ids = self.env['mrp.routing.workcenter'].search([])
    #     print(operation_ids,"1111111111111111111111111111111111111111")
    #     # if self.routing_id and config.process_costing == 'workcenter':
    #     if operation_ids and config.process_costing == 'workcenter':
    ...
    #             }
    #             labour_obj = self.env['mrp.bom.labour.cost'].create(value)
    ...
    #
    #             }
    #             overhead_obj = self.env['mrp.bom.overhead.cost'].create(value)
    ...
    #     if operation_ids and config.process_costing == 'workcenter':
    #         # self.bom_labour_cost_ids = False
    #         # self.bom_overhead_cost_ids = False
    ...
    #             # self.update({'bom_labour_cost_ids': [(6, 0, value)]})
    #
    #         for operation in operation_ids:
    #             value = {
    #                 'operation_id': operation.id,
    ...
    #             # self.update({'bom_overhead_cost_ids': [(6, 0, value)]})
    #     return res
...
class MrpProduction(models.Model):
    ...
    def create(self, vals):
        ...
        print(res,"444444444444444444444444")
        print(res.bom_id.bom_labour_cost_ids,"5555555555555555555555555555")
...
class MrpWorkorder(models.Model):
    ...
    def _compute_total_cost(self):
        ...
        for order in self:
            print(order.wo_material_cost_ids,"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaqqqqqqqqqqqqqqqqqq")
    ...
    def record_production(self):
        print(self.production_id.pro_labour_cost_ids,"llllllllllllllllllllllllllllll")
        for line in self.production_id.pro_labour_cost_ids:
            print(line.operation_id.id,"ggggggggggggggggggggggggggg")
            print(self.operation_id.id,"hhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh")
...
#                         material.write({
#                             'planned_qty': new_production_qty * bom.product_qty,
#                             'actual_qty': new_production_qty * bom.product_qty
#                         })
#
...
#                         labour.write({
#                             'planned_qty': new_production_qty * bom.planned_qty,
#                             'actual_qty': new_production_qty * bom.planned_qty
#                         })
#
...
#                 print(bom.operation_ids,"2222222222222222222222222222")
#                 # for operation in bom.routing_id.operation_ids:
#                 for operation in operation_ids:
#                     operation_bom_qty[operation.id] = bom_data['qty']
#
#             finished_moves_modification = self._update_product_to_produce(
#                 production, new_production_qty - qty_produced, old_production_qty)
#
...
#                     wo.state = 'done'
#
#                 # Assign moves; last operation receives all unassigned moves
...
#         return {}
    ...
    #             print(production,"xxxxxxxxxxxxxxxxxxxxxxxxxxx")
